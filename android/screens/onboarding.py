...
class OnboardingScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(MDLabel(text="Welcome!", halign="center", font_style="H4"))
    ...
    def on_submit(self, *args):
        ...
        print(f"[DEBUG] Fields - Name: '{name}', Email: '{email}', Password: {'*' * len(password)}")
        ...
        if self.is_signup and (not name or len(password) < 8):
            toast("Name required and password must be at least 8 chars")
            return
            ...
            def signup_thread():
                ...
                async def signup():
                    try:
                        ...
                        import traceback
                        print("[DEBUG] Signup exception:", traceback.format_exc())
            ...
            def login_thread():
                ...
                async def login():
                    try:
                        ...
                        import traceback
                        print("[DEBUG] Login exception:", traceback.format_exc())
                        Clock.schedule_once(lambda dt: toast(f"Error: {e}"))
                        Clock.schedule_once(lambda dt: setattr(self.submit_btn, 'disabled', False))
